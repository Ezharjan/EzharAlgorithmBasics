...
void inOrder_nonrecursion(Bitree *root)//非递归中序遍历
{
    stack<Bitree*> s;//栈中存储输入的字符串数组s 
    Bitree *p=root;//声明Bitree类型的指针变量p并初始化 
    while(p!=NULL||!s.empty())//当p的值不为空或者调用判断是否为空的函数内部值不为空 
    {
        while(p!=NULL)//如果p不为空 
        {
            s.push(p);//调用压入栈的函数 
            p=p->left_child;//将p指向的左孩子节点的存储空间赋给p 
        }
        if(!s.empty())//如果栈中的s不是空的 
        {
            p=s.top();//将栈顶元素赋给p 
            cout<<p->x<<" ";//输出x 
            s.pop();//s出栈 
            p=p->right_child;//将右孩子节点的存储空间指向p 
        }
    }    
} 
...
int main()
{
	...
	//（不论串的长短如何），scanf()函数的值总是1。这是c++里自带的一个函数。 
    {
		...
		cout<<"       ";//为界面美观起见 
        cout<<"非递归方式输出的中序遍历为："; 
        inOrder_nonrecursion(root);//非递归调用方式 
    }
    return 0;
}
